...
$(document).ready(function() {
	...
	document.getElementById('frmTestAccept').onload = function() {
		screenInit();
		getTeamInfo();
	}
...
function screenInit(){
	...
	tab = $('#frmTestAccept').get(0).contentWindow;
...
function btnQry_Click(num){
	...
	tmpInfo.recvdept= getSelectedVal('cboTeam').value;
	tmpInfo.secuyn	= 'Y';
	if(qryGbn == '00'){
...
function tabnavi_on(){
	...
	if (tab.strIsrId == selectedGridItem.cc_isrid && tab.strIsrSub == selectedGridItem.cc_isrsub) return;
	
	tab.screenInit('M');    
	tab.strIsrId = selectedGridItem.cc_isrid;
	tab.strIsrSub = selectedGridItem.cc_isrsub;
	tab.strSubStatus = selectedGridItem.cc_substatus;
	tab.strUserId = userId;
	tab.strReqCd = strReqCd;
	
//	if (selectedGridItem.cc_testeryn == "Y") tab.txtTestYn.text = "YES";
//	else tab1.txtTestYn.text = "YES"; //플렉스ver 코드 중복
	tab.$('#txtTestYn').val('YES'); 
	
	tab.$('#btnReg').prop('disabled', true);
