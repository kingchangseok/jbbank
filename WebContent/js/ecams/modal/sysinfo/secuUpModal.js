...
function jobValCheck() {
	...
	if(code.length <= 0) {
		dialog.alert('취약점 대상확장자를 입력해주세요.',function() {});
		return;
	}
	
	if(code.indexOf('.') < 0){
		dialog.alert('대상확장자 . 을 포함하여 입력해주세요.',function() {});
		return;
	}
	...
	if(selCode == null){
		requestType = "inSecuExe";
	} else {
		requestType = "upSecuExe";
