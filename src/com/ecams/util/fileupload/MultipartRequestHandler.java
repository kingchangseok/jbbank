	...
	public static List<FileMeta> uploadByJavaServletAPI(HttpServletRequest request) throws IOException, ServletException{
		...
		for(Part part:parts){
			...
			if(part.getContentType() != null){
				if (null!=noticeAcptno && !"".equals(noticeAcptno)) {
...
//			factory.setSizeThreshold(1024*1024*1024*1024*1);
			...
			// 임시파일 저장위치 설정
			File tmpFolder = new File(systemPath.getTmpDir("99"));
			if(!tmpFolder.exists()) {
				tmpFolder.mkdirs();
			}
			...
			for(FileItem item:items) {
		        ...
		        }else if(item.getFieldName().equals("uploadCnt")) {
		        	uploadCnt = Integer.parseInt(item.getString());
		        }
			}
			int seqno = cmm2101.getLastSeqno(noticeAcptno) + uploadCnt;
			//int seqno = 0;
			...
			for(FileItem item:items){
			    ...
			    if (item.isFormField()) {
					...
					temp.setSaveName(noticeAcptno + "."+seqno);
					temp.setSeq(Integer.toString(seqno));
...
//					File fileUploadFolder = new File(fileUploadDir + File.separator + noticeAcptno.substring(0,4)  + File.separator + noticeAcptno);
					File fileUploadFolder = new File(fileUploadDir + File.separator + noticeAcptno.substring(0,4));
					if(!fileUploadFolder.exists()) {
						fileUploadFolder.mkdirs();
					}

					//fileUploadFolder.deleteOnExit();
					fileUploadFolder = null;
					...
					if (fileName != null && 
					   (fileName.indexOf("..") > -1 || fileName.indexOf("&") > -1 || fileName.indexOf(">") > -1 || fileName.indexOf("<") > -1 ||
					    ...
					    throw new Exception ("첨부파일명, 파일경로를 확인해 주세요.");
					}
					
					if (fileName.getBytes().length > 49) {
					    throw new Exception ("파일명이 너무 깁니다. (50byte)");
					}
					...
					/*if(null != extCk && !"OK".equals(extCk)) {
					    throw new Exception ("허용되지않는 첨부파일입니다.");
					}*/

					// 경로 나중에 db에서 받아와서 셋팅

					String fileFullName = fileUploadDir + File.separator + noticeAcptno.substring(0,4)  + File.separator + noticeAcptno + "."+seqno;
