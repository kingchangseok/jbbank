...
var cboQryGbnData = [
	...
	{cm_codename : "SR종료대상",	cm_micode : "01", cm_dateyn: "N"}
];
...
function loadiFrame() {
	...
	document.getElementById('frmSRComplete').onload = function() {
		load7Sw = true;
		tab7 = $('#frmSRComplete').get(0).contentWindow;
	}
}
...
function isrID_click(data) {
	console.log('[SRComplete.js] isrID_click ==>', data);
	srData = new Object();
	strIsrId = '';
	...
	if(data.isrproc.indexOf('69') >= 0) visibleObj('on', 'tab7');
...
function changeTabMenu() {
	if(document.getElementById("tab1").className == "on") {				// SR등록
	...
	}else if(document.getElementById("tab7").className == "on") {		// SR완료
		tab0 = $('#frmPrjList').get(0).contentWindow;
		tab7 = $('#frmSRComplete').get(0).contentWindow;
		...
		//if(tab7.srData.strIsrId == srData.strIsrId) return;
		
		tab7.strIsrId = srData.strIsrId;
		tab7.strStatus = tab0.firstGrid.list[tab0.firstGrid.selectedDataIndexs].cc_status;
		tab7.strIsrTitle = tab0.firstGrid.list[tab0.firstGrid.selectedDataIndexs].cc_reqtitle;
		tab7.strEditor = tab0.firstGrid.list[tab0.firstGrid.selectedDataIndexs].cc_lastupuser;
		tab7.strQryGbn = tab0.firstGrid.list[tab0.firstGrid.selectedDataIndexs].cm_micode;
		tab7.screenInit('M');
		if(srData.strIsrId != null && srData.strIsrId != '' && srData.strIsrId != undefined) {
			tab7.srendInfoCall();
		}
