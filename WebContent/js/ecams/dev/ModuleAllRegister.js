...
var reqCd 		= window.top.reqCd;			// reqCd
...
batchGrid.setConfig({
    ...
    columns: [
        {key: "sysmsg", 		label: "시스템명",  		width: '8%',	align: "left"},
        {key: "cr_rsrcname",	label: "프로그램명",  		width: '8%', 	align: "left"},
        {key: "jawon", 			label: "프로그램종류",  	width: '7%', 	align: "left"},
        {key: "cm_dirpath", 	label: "프로그램경로",  	width: '14%', 	align: "left"},
        {key: "cr_rsrcname2", 	label: "모듈명",  		width: '6%'},
        {key: "jawon2", 		label: "모듈종류",  		width: '7%', 	align: "left"},
        {key: "cm_dirpath2", 	label: "모듈경로",  		width: '14%', 	align: "left"},
        {key: "errmsg", 		label: "체크결과",  		width: '25%', 	align: "left"}
    ]
});
...
$(document).ready(function() {
	...
	$('#btnSaveExl').bind('click', function() {
		batchGrid.exportExcel('실행모듈일괄등록리스트.xls');
	});
	
	// 엑셀템플릿
	$('#btnExlTmp').bind('click', function() {
		fileDown(attPath+"/"+"module_excel_templet.xls","module_excel_templet.xls");
...
function request_Check_In() {
	...
	ajaxAsync('/webPage/ecmd/Cmd1000Servlet', data, 'json',successRequestCheckIn);
}
...
function startUpload() {
	...
	formData.append('fullName',tmpPath+userId+"_excel_eCmd1000.tmp");
...
function onUploadCompleteData(filePath){
	...
	headerDef.push("cm_dirpath");
	headerDef.push("cr_rsrcname2");
	headerDef.push("jawon2");
	headerDef.push("cm_dirpath2");
...
function getFileListExcel() {
	...
	ajaxAsync('/webPage/ecmd/Cmd1000Servlet', tmpData, 'json',successGetFileListExcel);
}
