...
//            };
            this.setColumnSort = function(_sortInfo) {
                if (typeof _sortInfo !== "undefined") {
                    this.sortInfo = _sortInfo;
                    GRID.header.applySortStatus.call(this, _sortInfo);
                    sortColumns.call(this, _sortInfo);
                } else {
                	if (typeof this.sortInfo !== "undefined") {
...
//        });
        ...
        var numbering = 0;
        sortInfoArray = sortInfoArray.filter(function(item) {
        	if (item == undefined) return false;
        	else if (item.order == undefined) return false;
        	else {
        		item.seq = numbering;
        		item.sortseq = numbering;
        		numbering++;
        		return true;
        	}
        });
...
//                        }
                        if (!U.isNothing(col.key) && !U.isNothing(col.colIndex) 
                        		&& (cfg.sortable === true || col.sortable === true) && col.sortable !== false) {
                        	var columnsortseq = "";
                    		if (colGroup[col.colIndex].sortseq == undefined || colGroup[col.colIndex].sort == undefined || !cfg.multiSort) {
                    			columnsortseq = "";
                    		} else columnsortseq = colGroup[col.colIndex].sortseq+1+" ";
                    		
                    		if(!cfg.multiSort) {
                    			_SS += '<span data-ax5grid-column-sort="' + col.colIndex + '" data-ax5grid-column-sort-order="' + (colGroup[col.colIndex].sort || "") + '" />'
                    		}else {
                    			_SS += columnsortseq + '<span data-ax5grid-column-sort="'+col.colIndex
                				+ '" data-ax5grid-column-sort-order="' + (colGroup[col.colIndex].sort || "") + '" />';
                    		}
                        }
//                      2022.06.29 end
                        return _SS
...
//    };
    ...
    var toggleSort = function toggleSort(_key) {
    	...
    	seq = 0;
   		...
   		sortInfo = clone(this.sortInfo);
   		
    	var findkey = false;
    	var allcnt = 0;
    	var nowseq = 0;

    	for (var key1 in sortInfo) {
    		if (key1 == _key) {
    			findkey = true;
    			if (sortInfo[key1].orderBy == undefined) {
    				sortOrder = "asc";
    			} else if (sortInfo[key1].orderBy == "asc") {
    				sortOrder = "desc";
    				seq++;
    			} else if (sortInfo[key1].orderBy = "desc") {
    				nowseq = sortInfo[_key].seq;
    				sortOrder = undefined;
//    				delete sortInfo[key1];
//    				continue;
    			}	
    			
    			sortInfo[_key].orderBy = sortOrder;
    			if (sortInfo[_key].sortseq == undefined) {
    				sortInfo[_key].seq = 0;
    				sortInfo[_key].sortseq = 0;
    			}
    		} else {    			
    			if(!this.config.multiSort) {
    				sortInfo[key1].seq = 0;
    				sortInfo[key1].sortseq = 0;
    				delete sortInfo[key1];
    				continue;
    				sortOrder = undefined;
    			}else {
        			...
        			if (sortInfo[key1].orderBy != undefined) seq++;
        			try {
        				if (sortInfo[key1].sortseq == null || sortInfo[key1].sortseq == undefined) {
        					sortInfo[key1].sortseq = sortInfo[key1].seq;
        				}
        			}catch(e){
        				sortInfo[key1].sortseq = sortInfo[key1].seq;
        			}
    	...
    	if (!findkey) {
    		...
    		if (sortOrder == "asc") {
    			sortInfo[_key].seq = seq;
    			sortInfo[_key].sortseq = seq;
    		} else if (sortOrder == undefined) {
    			for (var key1 in sortInfo) {
    				if (sortInfo[key1].orderBy != undefined) {
    					if (nowseq != undefined && sortInfo[key1].seq > nowseq) {
    						var mineSeq = sortInfo[key1].seq-1;
    						sortInfo[key1].seq = mineSeq;
    						sortInfo[key1].sortseq = mineSeq;
    					}
    				}
    			}
    		}
...
//    2022.06.29 end
    ...
    var applySortStatus = function applySortStatus(_sortInfo) {
        for (var i = 0, l = this.colGroup.length; i < l; i++) {
        	if(!this.config.multiSort) {
        		this.colGroup[i].sort = '';
            	this.colGroup[i].sortseq = '';
        	}
            for (var _key in _sortInfo) {
                if (this.colGroup[i].key == _key) {
                    this.colGroup[i].sort = _sortInfo[_key].orderBy;
//                    2022.06.29 start
                    if (null != _sortInfo[_key].sortseq && _sortInfo[_key].sortseq != undefined) {
                    	this.colGroup[i].sortseq = _sortInfo[_key].sortseq;
                    }
//                    2022.06.29 end
                }
            }
