...
function successGeteCAMSDir(data) {
	...
	if (tmpArray.length > 0) {
		for (var i = 0; tmpArray.length > i; i++) {
			if (tmpArray.cm_pathcd == "F2") {
				strUpURL = tmpArray.cm_path;
			} else if (tmpArray.cm_pathcd == "21") {
				attPath = tmpArray.cm_path;
			}
		}
	} else {
...
function successGetRfcCnt(data) {
	...
	$('[data-ax5select="cboCnt"]').ax5select({
		options : injectCboDataToArr(cboCnt_dp, '0', 'rfcseq')
	});

	if(cboCnt_dp.length > 0) {
		$('[data-ax5select="cboCnt"]').ax5select("setValue", "0", true);
		cboCnt_click();
	}	
}
...
function cboCnt_click() {
	...
	$('#txtSta').val(cboCnt_dp[0].sta);
	if (cboCnt_dp[0].cc_status != "X") {			
		$('#txtCreatdt').val(cboCnt_dp[0].rfcreqdt);
		$('#txtRfcEndDt').val(cboCnt_dp[0].rfceddate);
		$('#txtEditor').val(cboCnt_dp[0].cm_username);
		$('#txtEXPRUNTIME').val(cboCnt_dp[0].cc_expruntime);			
		$('#txtEXPDAY').val(cboCnt_dp[0].cc_expday.substr(0,4)+"/"+cboCnt_dp[0].cc_expday.substr(4,2)+"/"+cboCnt_dp[0].cc_expday.substr(6,2));
		$('#datEXPDAY').val(cboCnt_dp[0].cc_expday.substr(0,4)+"/"+cboCnt_dp[0].cc_expday.substr(4,2)+"/"+cboCnt_dp[0].cc_expday.substr(6,2));  
		$('#txtEXPMM').val(cboCnt_dp[0].cc_expmm);
		if (cboCnt_dp[0].cc_exprsc != null) {
			$('#chkEXPRSC').prop('checked', true);
		}
		$('#txtEXPRSC').val(cboCnt_dp[0].cc_exprsc);
		
		if (cboCnt_dp[0].cc_acptno != "" && cboCnt_dp[0].cc_status != "0") {
	...
	if (cboCnt_dp[0].secusw == "Y") {
		...
		if (cboCnt_dp[0].cc_substatus !='1B' && cboCnt_dp[0].cc_substatus !='19'){
			$('#txtEXPDAY').css('visibility', 'hidden');
			// $('#lbEXPRSC').css('visibility', 'hidden');
			$('#datEXPDAY').css('visibility', 'visible');
			$('#cal1').css('visibility', 'visible');
	...
	if (cboCnt_dp[0].cc_status == "1") {
		//Cmr3100.gyulChk(cboCnt_dp[0].cc_acptno,strUserId);
		var data = {
			AcptNo : cboCnt_dp[0].cc_acptno,
			UserId : strUserId,
			requestType : 'gyulChk'
		}
...
function btnSave_click() {
	...
	etcObj.gbncd = "SAVE";
	etcObj.seqno = cboCnt_dp[0].cc_seqno;
...
function confOk(state) {
	if (state == 'close') {
		   var grdObj = new Object();
		   grdObj.AcptNo = cboCnt_dp[0].cc_acptno;
		   grdObj.UserId = strUserId;
