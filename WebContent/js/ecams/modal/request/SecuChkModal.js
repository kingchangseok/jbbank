...
//	checkStatus();
	...
	firstGrid.setConfig({
	     ...
	     columns: [
	        ...
	        {key: "cm_contents", label: "점검항목",  width: '40%', align: 'left', multiLine: true},
	...
	$(".close, #close").bind('click', function() {
		window.parent.secuChkModal.close();
	});
...
function checkStatus() {
	if(window.parent.reqInfoData[0].prcdate != null && window.parent.reqInfoData[0].prcdate != "") {
		endSw = true
	} else {
		endSw = false;
	}
...
function cmdReq() {
	...
	$.each(gridData, function(i, val) {
		...
		if(val.cm_chkrst != "P" && (tmpCmt == null || tmpCmt == "")) {
			dialog.alert("[" + val.cm_contents + "]\n항목에 대한 사유를 입력하여 주십시오.");
			tmp_obj = null;
			errSw = true;
			return false;
		}
		...
		tmp_obj.cm_gbncd = val.cm_gbncd;
		tmp_obj.cm_chkrst = val.cm_chkrst;
	...
	if(!errSw) {		
		window.parent.chkListData = tmpArr;
		window.parent.secuChkModal.close();
	} else {
		tmpArr = null;
	}
