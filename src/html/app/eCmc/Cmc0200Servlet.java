...
@WebServlet("/webPage/ecmc/Cmc0200Servlet")
public class Cmc0200Servlet extends HttpServlet {
 ...
 protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			switch (requestType) {
			case "setISRSUBInfo":
				response.getWriter().write(setISRSUBInfo(jsonElement));
				break;
			case "getIsrInfo":
	...
	*/ 
	private String setISRSUBInfo(JsonElement jsonElement) throws SQLException, Exception {
		Cmc0200 cmc0200 = new Cmc0200();
		try {
		    HashMap<String,String> etcData = ParsingCommon.jsonStrToMap(ParsingCommon.jsonEtoStr(jsonElement, "etcData"));
		    return ParsingCommon.toJson(cmc0200.setISRSUBInfo(etcData));
		} catch (IOException e) {
	...
	*/ 
	private String getIsrInfo(JsonElement jsonElement) throws SQLException, Exception {
		...
		try {
		    ...
		    return ParsingCommon.toJson(cmc0200.getIsrInfo(IsrId,IsrSub,UserId,ReqCd));
		} catch (IOException e) {
		    return ParsingCommon.toJson("ERROR"+e.getMessage());
		} catch (Exception e) {
			return ParsingCommon.toJson("ERROR"+e.getMessage());
	...
	*/ 
	private String getHandlerList(JsonElement jsonElement) throws SQLException, Exception {
		...
		try {
		    String UserId = ParsingCommon.jsonStrToStr(ParsingCommon.jsonEtoStr(jsonElement, "UserId"));
		    String ReqCd = ParsingCommon.jsonStrToStr(ParsingCommon.jsonEtoStr(jsonElement, "ReqCd"));
		    return ParsingCommon.toJson(cmc0200.getHandlerList(UserId,ReqCd));
		} catch (IOException e) {
		    return ParsingCommon.toJson("ERROR"+e.getMessage());
		} catch (Exception e) {
			return ParsingCommon.toJson("ERROR"+e.getMessage());
