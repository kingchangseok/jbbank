...
import com.ecams.common.base.HttpCall_New;
...
public class SRRestApi {
	private com.google.gson.JsonObject rootJson;
    /**
	...
	public String setError(String errMsg) throws Exception{
		...
		try {
			if (errMsg != null) {
				...
				if (!s.isEmpty()) {
					int nPos = s.indexOf(':');
					if (nPos > 0 && nPos < s.length() - 1) {
						code = s.substring(0, nPos).trim();
						message = s.substring(nPos + 1).trim();
					} else {
						java.util.regex.Matcher m = java.util.regex.Pattern
								.compile("^(ERIN\\w+)[\\s-]*(.*)$")
								.matcher(s);
						if (m.find()) {
							code = m.group(1);          // "ERINXX"
							String rest = m.group(2);   // "SrDevReqNo Field Empty"
							message = (rest != null && !rest.trim().isEmpty()) ? rest.trim() : "Unknown error";
						} else {
							message = s;
						}
					}
			...
			jsonObj.addProperty("ResultCd", code);
			jsonObj.addProperty("ResultMsg", message);
		} catch(Exception exception) {
			exception.printStackTrace();
			ecamsLogger.error("## SRRestApi.setError() Exception START ##");				
			ecamsLogger.error("## Error DESC : ", exception);	
			ecamsLogger.error("## SRRestApi.setError() Exception END ##");	
			jsonObj.addProperty("ResultCd", "ERINXX");
			jsonObj.addProperty("ResultMsg", "setError failure: " + String.valueOf(exception));
		}
    ...
    public String setError_back(String errMsg) throws Exception{
    	String jsonStr = "";
    	String convStr = "";
    ...
    // keep track of incoming SR number for error context
    String srDevNo = "";
    String returnMsg = "";		
    String itemData = "";
    	try {    		
    		if (trCode == null || "".equals(trCode)) returnMsg = "ERIN01:TrCode Field Empty";
    		else if (inData == null || "".equals(inData)) returnMsg = "ERIN02:InputData Field Empty";
            else {
                jsonObj = (JsonObject) jsonparser.parse(inData);
                // capture SrDevReqNo early to include in error messages
                try {
                    if (jsonObj.has("SrDevReqNo")) {
                        srDevNo = jsonObj.get("SrDevReqNo").getAsString();
                    }
                } catch(Exception __e_sr) {
                    // ignore
                }
                if ("CC".equals(trCode.substring(0,2))) {
                    if (!jsonObj.has("SrDevReqNo")) returnMsg = "ERIN03:SrDevReqNo Field Empty";
                }

                // 신규 공통 검증 : TestRequestYn / TestPassingCd (적용 범위: CC_REQ_REG, CC_REQ_UPD)
                if ("CC_REQ_REG".equals(trCode) || "CC_REQ_UPD".equals(trCode)) {
					if (returnMsg.length()==0) {
						// 1) TestRequestYn 필드 존재 여부
						boolean hasTestYn = jsonObj.has("TestRequestYn") ;
						if (!hasTestYn) {
							returnMsg = "ERIN40:TestRequestYn Field Empty ["+trCode+"-TestRequestYn]";
						}
					}
					if (returnMsg.length()==0) {
						// 2) TestRequestYn 값이 'N'인 경우 TestPassingCd 필수
						String _testYn = null;
						if (jsonObj.has("TestRequestYn")) {
							_testYn = jsonObj.get("TestRequestYn").getAsString();
						}
						if (_testYn != null && "N".equalsIgnoreCase(_testYn)) {
							if (!jsonObj.has("TestPassingCd")) {
								returnMsg = "ERIN41:TestPassingCdNoData ["+trCode+"-TestPassingCd(TestRequestYn=N)]";
							}
						}
					}
                	// 신규 공통 검증 끝
    		...
    		if (returnMsg.length()==0) { 
				if ("CC_REQ_REG".equals(trCode)) { //개발요청서 등록
					if (jsonObj.has("RequestDoc")) { //개발요청서
						...
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("DocType")) returnMsg = "ERIN05:Missing RequestDoc.DocType field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("DocNo")) returnMsg = "ERIN07:Missing RequestDoc.DocNo field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("DocSubject")) returnMsg = "ERIN08:Missing RequestDoc.DocSubject field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("ReqDocNo")) returnMsg = "ERIN09:Missing RequestDoc.ReqDocNo field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("BizManager")) returnMsg = "ERIN10:Missing RequestDoc.BizManager field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("JobCd")) returnMsg = "ERIN11:Missing RequestDoc.JobCd field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("Contents")) returnMsg = "ERIN12:Missing RequestDoc.Contents field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("StartDt")) returnMsg = "ERIN13:Missing RequestDoc.StartDt field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("EndDt")) returnMsg = "ERIN14:Missing RequestDoc.EndDt field";
						}
						if (returnMsg.length()==0) {
                            if (!jsonObj2.has("EmpNo")) returnMsg = "ERIN15:Missing RequestDoc.EmpNo field";
						}
						if (returnMsg.length()==0) {
							if (jsonObj2.has("OrderDoc")) {
								jsonObj3 = jsonObj2.getAsJsonObject("OrderDoc");
                                    if (!jsonObj3.has("EndPlanDt")) returnMsg = "ERIN16:Missing RequestDoc.OrderDoc.EndPlanDt field";
								else {
									itemData = jsonObj3.get("EndPlanDt").toString().replace("\"", "");
			    					if (itemData.length()!=10) returnMsg = "ERIN17:WorkOrderDueDt format mismatch["+itemData+"]";
								}
								if (returnMsg.length()==0) {
                                    if (!jsonObj3.has("Referencor")) returnMsg = "ERIN18:Missing RequestDoc.OrderDoc.Referencor field";
								}
								if (returnMsg.length()==0) {
                                    if (!jsonObj3.has("Developer")) returnMsg = "ERIN19:Missing RequestDoc.OrderDoc.Developer field";
								}
								if (returnMsg.length()==0) {
                                    if (!jsonObj3.has("EmpNo")) returnMsg = "ERIN20:Missing RequestDoc.OrderDoc.EmpNo field";
								}
							}
						}
					}
					if (jsonObj.has("SmRequestDoc")) { //외주개발요청서
						...
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("DocType")) returnMsg = "ERIN31:Missing SmRequestDoc.DocType field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("DocNo")) returnMsg = "ERIN32:Missing SmRequestDoc.DocNo field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("JobCd")) returnMsg = "ERIN11:Missing SmRequestDoc.JobCd field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("BizManager")) returnMsg = "ERIN10:Missing SmRequestDoc.BizManager field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("DocSubject")) returnMsg = "ERIN12:Missing SmRequestDoc.DocSubject field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("Contents")) returnMsg = "ERIN12:Missing SmRequestDoc.Contents field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("StartDt")) returnMsg = "ERIN13:Missing SmRequestDoc.StartDt field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("EndDt")) returnMsg = "ERIN14:Missing SmRequestDoc.EndDt field";
						}
						if (returnMsg.length()==0) {
                        	if (!jsonObj2.has("EmpNo")) returnMsg = "ERIN15:Missing SmRequestDoc.EmpNo field";
						}
						if (returnMsg.length()==0) {
							if (jsonObj2.has("OrderDoc")) {
								jsonObj3 = jsonObj2.getAsJsonObject("OrderDoc");
                                if (!jsonObj3.has("EndPlanDt")) returnMsg = "ERIN16:Missing SmRequestDoc.OrderDoc.EndPlanDt field";
								else {
									itemData = jsonObj3.get("EndPlanDt").toString().replace("\"", "");
			    					if (itemData.length()!=10) returnMsg = "ERIN17:WorkOrderDueDt format mismatch["+itemData+"]";
								}
								if (returnMsg.length()==0) {
                                    if (!jsonObj3.has("Referencor")) returnMsg = "ERIN18:Missing SmRequestDoc.OrderDoc.Referencor field";                                     
								}
								if (returnMsg.length()==0) {
                                    if (!jsonObj3.has("Developer")) returnMsg = "ERIN19:Missing SmRequestDoc.OrderDoc.Developer field";  
								}
								if (returnMsg.length()==0) {
                                    if (!jsonObj3.has("EmpNo")) returnMsg = "ERIN20:Missing SmRequestDoc.OrderDoc.EmpNo field";  
								}
							}
						}
					...
					if (jsonObj.has("RequestDoc")) { //개발요청서
						...
						if (returnMsg.length()==0) {
							if (jsonObj2.has("OrderDoc")) {
								jsonObj3 = jsonObj2.getAsJsonObject("OrderDoc");
								if (returnMsg.length()==0) {
                                	if (!jsonObj3.has("EndPlanDt")) returnMsg = "ERIN18:Missing RequestDoc.OrderDoc.EndPlanDt field";
								}
								if (returnMsg.length()==0) {
                                	if (!jsonObj3.has("Referencor")) returnMsg = "ERIN18:Missing RequestDoc.OrderDoc.Referencor field";
								}
					...
					if (jsonObj.has("SmRequestDoc")) { //외주개발요청서
						...
						if (returnMsg.length()==0) {
							if (jsonObj2.has("OrderDoc")) {
								jsonObj3 = jsonObj2.getAsJsonObject("OrderDoc");
								if (returnMsg.length()==0) {
                                    if (!jsonObj3.has("EndPlanDt")) returnMsg = "ERIN18:Missing SmRequestDoc.OrderDoc.EndPlanDt field";                                     
								}
								if (returnMsg.length()==0) {
                                	if (!jsonObj3.has("Referencor")) returnMsg = "ERIN18:Missing SmRequestDoc.OrderDoc.Referencor field";                                      
								}
        ...
        else if (returnMsg != null && returnMsg.startsWith("ERIN")) {
            // include SrDevReqNo in the error message if available
            if (srDevNo != null && srDevNo.length() > 0) {
                return setError(returnMsg + " (SrDevReqNo:" + srDevNo + ")");
				/*  오류 리턴 예시
				{"ResultCd": "ERIN40", "ResultMsg": "TestRequestYn Field Empty [CC_REQ_REG-TestRequestYn] (SrDevReqNo:SR2025-1234)"}
				*/
            } else {
                return setError(returnMsg);
            }
        }
        return returnMsg;
    }
    ...
    public ArrayList<HashMap<String,String>> dataConv(String trCode,String itemName,JsonObject jsonObj,Connection conn) throws Exception{
    	...
    	try {
	        ...
	        // ======================================================================
	        // 데이터 컨버전 공통 선처리(루트 테스트 필드)
	        // ======================================================================
	        String rootTestYn = "N";
	        String rootTestCd = "00";
	        if (this.rootJson != null) {
	            // TestRequestYn(정상) / TestRequsetYn(오탈자) 우선 처리
	            try {
	                if (this.rootJson.has("TestRequestYn")) {
	                    String v = this.rootJson.get("TestRequestYn").getAsString();
	                    if ("Y".equalsIgnoreCase(v)) rootTestYn = "Y";
	                }
	            } catch (Exception __e1) { /* 기본값 유지 */ }
	            try {
	                if (this.rootJson.has("TestPassingCd")) {
	                    String v = this.rootJson.get("TestPassingCd").getAsString();
	                    if (v != null && v.trim().length() > 0) rootTestCd = v.trim();
	                }
	            } catch (Exception __e2) { /* 기본값 유지 */ }
	        }
	        
			if ("CC_REQ_REG".equals(trCode)) { //개발요청서 등록
				if ("RequestDoc".equals(itemName) || "SmRequestDoc".equals(itemName)) { //개발요청서	
					etcData.put("CC_TESTREQ_YN", rootTestYn);
					etcData.put("CC_TESTPASS_CD", rootTestCd);
					...
					//itemData = new String(jsonObj.get("DocSubject").toString().getBytes("ISO-8859-1"),"UTF-8");     //문서제목
					//itemData = new String(jsonObj.get("DocSubject").toString().getBytes("EUC-KR"),"UTF-8");     //문서제목
					...
					*/
					if (jsonObj.has("EmpNo")) {  // 담당자
                        String empNo = jsonObj.get("EmpNo").toString().replace("\"", "");
                        etcData.put("CC_EDITOR", empNo);
						addData.put("CC_EDITOR", empNo);
                        ...
                        if (rs.next()) {
                            ...
                            errMap.put("errsw", "Y");
                            // SrDevReqNo가 있으면 메시지에 덧붙임
							String srDevNo = "";
							if (jsonObj.has("SrDevReqNo")) {
								srDevNo = jsonObj.get("SrDevReqNo").getAsString();
								if (srDevNo == null) srDevNo = "";
								else srDevNo = srDevNo.trim();
							}
							//String msg = "ERCM40: Not Found in CMM0040 [cm_userid=" + empNo + "] or cm_project null";
							String msg = "ERCM40:형상관리시스템에 사용자정보가 등록되지 않았습니다. 확인 후 재등록하시기 바랍니다. 사용자행번[" + empNo + "]" ;

                            if (srDevNo != null && srDevNo.length() > 0) {
                                msg = msg + " (SrDevReqNo:" + srDevNo + ")";
                            }
                            // 통일된 메시지 키
                        ...
                        etcData.put("CC_ETTEAM", itemData); // 담당팀(cm_project)
                        addData.put("CC_ETTEAM", itemData); // 담당팀(cm_project)
					} else {
					...
					*/
					...
					if ("RequestDoc".equals(itemName)){
						...
						if ("02".equals(etcData.get("CC_DOCTYPE")) || "99".equals(etcData.get("CC_DOCTYPE"))) {  //문서유형이 대외   
							...
							rs = pstmt.executeQuery();
							if (rs.next()) {
								//itemData = etcData.get("CC_DOCTYPE");
							...
							rs.close();
							pstmt.close();
						}else if ("04".equals(etcData.get("CC_DOCTYPE"))) {  //문서유형이 IT부서 자체발행
							strQuery.setLength(0);
							strQuery.append("select cm_deptname from cmm0100 \n");
							strQuery.append(" where cm_deptcd=?              \n");
							pstmt = conn.prepareStatement(strQuery.toString());
					...
					if (returnMsg.length()==0) {
						for (i=0;jsonAry.size()>i;i++) {
							...
							*/
							...
							if (jsonObj2.has("EmpNo")) {
								String empNo = jsonObj2.get("EmpNo").toString().replace("\"", "");
								etcData.put("cm_userid", empNo);
						...
						if (jsonObj2.has("Referencor")) {
							...
							if (jsonAry != null) {
								for (i=0;jsonAry.size()>i;i++) {
									...
									if (jsonObj3.has("EmpNo")) {
										String empNo = jsonObj3.get("EmpNo").toString().replace("\"", "");
										etcData.put("cm_userid", empNo);
										...
										if (rs.next()) {
											...
											if (!srDevNo.isEmpty()) msg += " (SrDevReqNo:" + srDevNo + ")";
											errMap.put("errmsg", msg);
									...
									if (jsonObj3.has("EmpNo")) {
										...
										if (rs.next()) {
											...
											try {
												if (jsonObj.has("SrDevReqNo")) {
													srDevNo = jsonObj.get("SrDevReqNo").getAsString();
													if (srDevNo == null) srDevNo = ""; else srDevNo = srDevNo.trim();
												}
				...
				if ("RequestDoc".equals(itemName) || "SmRequestDoc".equals(itemName)) { //개발요청서
					addData.put("CC_TESTREQ_YN", rootTestYn);
					addData.put("CC_TESTPASS_CD", rootTestCd);

					if (jsonObj.has("SrDevReqNo")) {  //담당자
						addData.put("strSRReqID", itemData);
					}
					ecamsLogger.error("SRRestApi dataconv CC_REQ_UPD rootTestYn >"+ rootTestYn + " rootTestCd > " + rootTestCd);
...
*/					
					if (jsonObj.has("EmpNo")) {  // 담당자
                        ...
                        ecamsLogger.error("SRRestApi dataconv1= empNo "+ empNo);
                        ...
                        ecamsLogger.error("SRRestApi dataconv2= empNo null ");
					...
					if (addData.get("CC_DEVSTDT") != null && !"".equals(addData.get("CC_DEVSTDT")) && 
						addData.get("CC_DEVEDDT") != null && !"".equals(addData.get("CC_DEVEDDT"))) {
						cmm1000 = new Cmm1000();
						int dateGap = cmm1000.getHoliDayAll_conn(addData.get("CC_DEVSTDT"),addData.get("CC_DEVEDDT"),conn);
						cmm1000 = null;
						addData.put("CC_DEVPERIOD",Integer.toString(dateGap));
					}
					/*  20251014 END */
							...
							if (jsonObj2.has("Referencor")) {
								...
								if (jsonAry != null) {
									for (i=0;jsonAry.size()>i;i++) {
										...
										etcData.put("CC_ETTEAM", addData.get("CC_ETTEAM"));
										...
										etcData.put("CC_DEVPERIOD", addData.get("CC_DEVPERIOD"));
										etcData.put("CC_ENDPLAN", addData.get("CC_ENDPLAN"));
										etcData.put("CC_TESTREQ_YN", addData.get("CC_TESTREQ_YN"));
										etcData.put("CC_TESTPASS_CD", addData.get("CC_TESTPASS_CD"));
										...
										*/
										...
										if (jsonObj3.has("EmpNo")) {
											...
											if (rs.next()) {
												...
												//String msg = "ERCM40: Referencor Not Found in CMM0040 [cm_userid=" + empNo + "] or cm_project null";
												String msg = "ERCM40:형상관리시스템에 사용자정보가 등록되지 않았습니다. 확인 후 재등록하시기 바랍니다. 사용자행번[" + empNo + "]" ;
										...
										*/
										...
										if (jsonObj3.has("EmpNo")) {
											...
											if (rs.next()) {
												...
												//String msg = "ERCM40: Developer Not Found in CMM0040 [cm_userid=" + empNo + "] or cm_project null";
												String msg = "ERCM40:형상관리시스템에 사용자정보가 등록되지 않았습니다. 확인 후 재등록하시기 바랍니다. 사용자행번[" + empNo + "]" ;
								...
								*/
								...
								if (jsonObj2.has("EmpNo")) {
									...
									if (rs.next()) {
										...
										//String msg = "ERCM40: BizManager Not Found in CMM0040 [cm_userid=" + empNo + "] or cm_project null";
										String msg = "ERCM40:형상관리시스템에 사용자정보가 등록되지 않았습니다. 확인 후 재등록하시기 바랍니다. 사용자행번[" + empNo + "]" ;
			...
			exception.printStackTrace();
			ecamsLogger.error("## SRRestApi.dataConv() Exception START ##");				
			ecamsLogger.error("## Error DESC : ", exception);	
			ecamsLogger.error("## SRRestApi.dataConv() Exception END ##");				
			returnMsg = "ERINX4:"+exception;
		}finally{
			try {
    ...
    public String setSRInfo(String inData) throws SQLException, Exception{
		...
		try {
			...
			if (!"OK".equals(retMsg)) retMsg = setError(retMsg);
				...
				// 루트 JSON을 dataConv에서 참조할 수 있도록 보관
				this.rootJson = jsonObj;
				...
				if (retMsg.length() == 0) {
					if (jsonObj.has("RequestDoc")) {
                        ...
                        if (rsval.size()==0) retMsg = "ER0101:Invalid registration data";
						else {
						    etcData = rsval.get(0);	
						
							if (etcData.get("errsw") != null && !"".equals(etcData.get("errsw"))) {
								retMsg = "ER0102:"+etcData.get("errmsg");
                            } else if (etcData.get("CC_REQID") == null || "".equals(etcData.get("CC_REQID"))) retMsg = "ER0103:Missing configuration assignment number";
						}
						...
						if (retMsg.length()==0) {
							if (jsonObj2.has("RecvDept") && strECAMSID1 != null && !"".equals(strECAMSID1)) {
                                ...
                                if (rsval.size()==0) retMsg = "ER0104:Invalid registration data";
								else {
						...
						if (retMsg.length()==0) {						
							if (jsonObj2.has("BizManager") && strECAMSID1 != null && !"".equals(strECAMSID1)) {
                                ...
                                if (rsval.size()==0) retMsg = "ER0106:Invalid registration data";
								else {
						...
						if (retMsg.length()==0) {						
							if (jsonObj2.has("OrderDoc") && strECAMSID1 != null && !"".equals(strECAMSID1)) {
                                ...
                                if (rsval.size()==0) retMsg = "ER0108:Invalid registration data";
								else {
					...
					if (jsonObj.has("SmRequestDoc")) {
                        ...
                        if (rsval.size()==0) retMsg = "ER0110:Invalid registration data";
						else {
						    etcData = rsval.get(0);	
						
							if (etcData.get("errsw") != null && !"".equals(etcData.get("errsw"))) {
								retMsg = "ER0111:"+etcData.get("errmsg");
                            } else if (etcData.get("CC_REQID") == null || "".equals(etcData.get("CC_REQID"))) retMsg = "ER0104:Missing configuration assignment number";
						}
						...
						if (retMsg.length()==0) {						
							if (jsonObj2.has("BizManager") && strECAMSID2 != null && !"".equals(strECAMSID2)) {
								rsval.clear();
								rsval = dataConv(trCode,"BizManager",jsonObj2,conn);
                                if (rsval.size()==0) retMsg = "ER0112:Invalid registration data";
								else {
							...
							if (retMsg.length()==0) {						
								if (jsonObj2.has("OrderDoc") && strECAMSID2 != null && !"".equals(strECAMSID2)) {
									rsval.clear();
									rsval = dataConv(trCode,"OrderDoc",jsonObj2,conn);
                                    if (rsval.size()==0) retMsg = "ER0114:Invalid registration data";
									else {
            ...
            if (retMsg.length()==0) {
                ...
                /* 오류 시, retMsg가 JSON 문자열이면 그대로 사용하고 아니면 setError 처리
                String errorJson;
                String trimmed = retMsg != null ? retMsg.trim() : "";
                if (trimmed.startsWith("{")) {
                    errorJson = trimmed;
                } else {
                    errorJson = setError(retMsg);
                }
                jsonStr = errorJson;
                */
                ...
                result.addProperty("ResultCD", "FAIL");
                result.addProperty("ResultMsg", retMsg.trim());
                result.addProperty("SrDevReqNo", strSRReqID);                
     ...
     */
			...
			if (!"OK".equals(retMsg)) retMsg = setError(retMsg);
				...
				if (retMsg.length() == 0) {
					if (jsonObj.has("RequestDoc")) {
                        ...
                        if (rsval.size()==0) retMsg = "ER0201:Invalid registration data";
						else {
						    etcData = rsval.get(0);	

							if (etcData.get("errsw") != null && !"".equals(etcData.get("errsw"))) {
								retMsg = "ER0202:"+etcData.get("errmsg");
                            } else if (etcData.get("CC_REQID") == null || "".equals(etcData.get("CC_REQID"))) retMsg = "ER0203:Missing configuration assignment number";
					...
					if (jsonObj.has("SmRequestDoc")) {
                        ...
                        if (rsval.size()==0) retMsg = "ER0204:Invalid registration data";
						else {
						    etcData = rsval.get(0);	
						
							if (etcData.get("errsw") != null && !"".equals(etcData.get("errsw"))) {
								retMsg = "ER0205:"+etcData.get("errmsg");
                            } else if (etcData.get("CC_REQID") == null || "".equals(etcData.get("CC_REQID"))) retMsg = "ER0206:Missing configuration assignment number";
        ...
        if (retMsg.length() == 0) {
            // 성공 시 결과 JSON 생성 (ResultCD/ResultMsg 포함)
            JsonObject result = new JsonObject();
            result.addProperty("ResultCD", "OK");
            result.addProperty("ResultMsg", "Success");
            result.addProperty("SrDevReqNo", strSRReqID);
            if (strECAMSID1 != null && !strECAMSID1.isEmpty()) {
                JsonObject rdoc = new JsonObject();
                rdoc.addProperty("RequestDocNo", strECAMSID1);
                rdoc.addProperty("RequestStatus", strECAMSSta1);
                if (strOrderID1 != null && !strOrderID1.isEmpty()) {
                    JsonObject ord1 = new JsonObject();
                    ord1.addProperty("OrderDocNo", strOrderID1);
                    ord1.addProperty("OrderStatus", strOrderSta1);
                    rdoc.add("OrderDoc", ord1);
                }
                result.add("RequestDoc", rdoc);
            }
            if (strECAMSID2 != null && !strECAMSID2.isEmpty()) {
                JsonObject sdoc = new JsonObject();
                sdoc.addProperty("SmRequestDocNo", strECAMSID2);
                sdoc.addProperty("SmRequestStatus", strECAMSSta2);
                if (strOrderID2 != null && !strOrderID2.isEmpty()) {
                    JsonObject ord2 = new JsonObject();
                    ord2.addProperty("OrderDocNo", strOrderID2);
                    ord2.addProperty("OrderStatus", strOrderSta2);
                    sdoc.add("OrderDoc", ord2);
                }
                result.add("SmRequestDoc", sdoc);
            }
            jsonStr = new Gson().toJson(result);
        } else {
            /* 오류가 발생한 경우 retMsg가 이미 JSON이면 그대로 사용하고, 아니면 setError로 JSON 변환
            String trimmed = (retMsg != null) ? retMsg.trim() : "";
            if (trimmed.startsWith("{")) {
                jsonStr = trimmed;
            } else {
                try {
                    jsonStr = setError(retMsg);
                } catch(Exception ex) {
                    JsonObject errObj = new JsonObject();
                    errObj.addProperty("ResultCd", "ERINXX");
                    errObj.addProperty("ResultMsg", "setError failure: " + String.valueOf(ex));
                    jsonStr = new Gson().toJson(errObj);
                }
                */
    ...
    public String insertCMC0400(HashMap<String, String> etcData,Connection conn) throws SQLException, Exception{
		...
		try {			
			...
			if (retMsg.length()==0) {
	        	...
	        	strQuery.append("     CC_STATUS,CC_EDITOR,CC_ETTEAM,CC_DEVPERIOD,CC_SRREQID,    				    \n");
	        	strQuery.append("     CC_TESTREQ_YN,CC_TESTPASS_CD ) values	    	                                \n");
	        	...
	        	pstmt.setString(pstmtcount++, etcData.get("CC_TESTREQ_YN"));
	        	if("Y".equals(etcData.get("CC_TESTREQ_YN"))){
		        	pstmt.setString(pstmtcount++, "00");  //해당없음
	        	}else{
	        		pstmt.setString(pstmtcount++, etcData.get("CC_TESTPASS_CD"));
	        	}
	        	ecamsLogger.error(((LoggableStatement)pstmt).getQueryString());
	        	pstmt.executeUpdate();
	        	pstmt.close();
    ...
    public String updateCMC0400(HashMap<String, String> etcData,Connection conn) throws SQLException, Exception{
		...
		try {			
        	...
        	strQuery.append("     CC_DETAILSAYU=?,CC_ENDPLAN=?,CC_SRREQID=?,CC_TESTREQ_YN=?,CC_TESTPASS_CD=?   	\n");        	
        	//strQuery.append("where CC_REQID = ?																		\n");
        	strQuery.append("where CC_SRREQID = ?																		\n");        	
        	pstmt = conn.prepareStatement(strQuery.toString());
			pstmt = new LoggableStatement(conn,strQuery.toString());
			pstmtcount = 1;
        	pstmt.setString(pstmtcount++, etcData.get("CC_DEVSTDT"));
			...
			for (i=0;rsval.size()>i;i++) {
				if ("biz".equals(rsval.get(i).get("gbn"))) {
					...
					if (rsval.get(i).get("Status") != null && !"".equals(rsval.get(i).get("Status")) && "00".equals(rsval.get(i).get("Status"))) {
			        	...
			        	//자체발행 체크(CMC0400 : CC_DOCTYPE = 03/04 인 경우) 
						strQuery.setLength(0);
			        	strQuery.append("select count(*) cnt		     \n");
			        	strQuery.append("   from cmc0400				 \n");
			        	strQuery.append(" where cc_reqid=?				 \n");
			        	strQuery.append("   and cc_doctype in('03','04') \n");		        	
			        	pstmt = conn.prepareStatement(strQuery.toString());
						//pstmt = new LoggableStatement(conn,strQuery.toString());
						pstmtcount = 1;
						pstmt.setString(pstmtcount++, strECAMSID);
			        	...
			        	if (!findSw) {   // 자체 발행건 (잘못된 요청서 들어오는거는 체크할수 없음)
			        		strQuery.setLength(0);
				        	...
				        	findSw      = false;
				        	project = rsval.get(i).get("cm_project");
				        	
				        	if(project != null && project.length()>=4){
				        		wkProject =  rsval.get(i).get("cm_project").substring(0,4);
				        	}
				        	...
				        	if(wkProject != null){
					        	...
					        	strQuery.append(" where cm_macode ='ITDEPT'		\n");
					        	strQuery.append("   and cm_closedt is null   	\n");	
					        	strQuery.append("   and cm_micode <> '****'   	\n");			        	
					        	pstmt = conn.prepareStatement(strQuery.toString());
					        	rs = pstmt.executeQuery();
					        	while (rs.next()) {
					        		if (rs.getString("cm_micode").equals(rsval.get(i).get("cm_project").substring(0,4))){
					        			ecamsLogger.error("SRRestApi BizManager_Cmc0401 IT Dept Check TRUE! >"+ rsval.get(i).get("cm_project"));				        			
					        			findSw = true;
					        		}
					        	} 
					        	rs.close();
					        	...
					        	if (!findSw) {   //IT부서가 아닌경우만 주관부서 담당자 추가
									strQuery.setLength(0);
						        	strQuery.append("insert into cmc0401					\n");
						        	strQuery.append("    (CC_REQID,CC_DEPT3,CC_REQUSER3)	\n");
						        	strQuery.append("values(?,?,?)			   			    \n");			        	
		...
		try {
			...
			if (conn == null) retMsg = "ERDB00:DB Connection Fail";
				...
				if (!AcptNo.substring(4,6).equals("16")){
					...
					/*  SYSTS 방어로직 추가   20251128 */
					strQuery.setLength(0);
					strQuery.append("select count(*) cnt   from cmr9900 a                                               \n");
					strQuery.append(" where cr_acptno=?                                                                 \n");
					strQuery.append(" and not exists (select 1 from cmr9900 x where x.cr_acptno = a.cr_acptno           \n");
					strQuery.append("                  and x.cr_locat ='00' and x.cr_team ='SYSTS')                     \n");
					strQuery.append(" and not exists (select 1 from cmr9900 x where x.cr_acptno = a.cr_acptno           \n");
					strQuery.append("                  and x.cr_team <>'SYSTS' and cr_teamcd = '1' and cr_status ='0')  \n");
					pstmt = conn.prepareStatement(strQuery.toString());	
					//pstmt = new LoggableStatement(conn,strQuery.toString());
					pstmt.setString(1,AcptNo);
					rs = pstmt.executeQuery();
					...
					if (!"F".equals(wkFg)){
						...
						jsonObj = new JsonObject();
						strQuery.setLength(0);
						strQuery.append("select to_char(a.cr_acptdate,'yyyy-mm-dd hh24:mi:ss') cr_acptdate,  \n");
						strQuery.append("       nvl(to_char(cr_prcdate, 'yyyy-mm-dd hh24:mi:ss'),to_char(sysdate,'yyyy-mm-dd hh24:miss')) cr_prcdate,   \n");
						...
						/*  SRID 번호 없는 경우 배포이력 API 호출 여부 체크 로직 여기서 넣으면 됨  2025/12/03 
						 *  하단의 cc_srreqid가 없는 경우 API 호출 안하도록 수정하면 됨
						 *  */
    ...
    public String callTSRResult(String AcptNo) throws SQLException, Exception{
 		//gbnCd  1:테스트적용결과/2:운영적용결과/3:테스트완료여부검증 하고자 함
 		...
 		try {
 			...
 			if (conn == null) retMsg = "ERDB00:DB Connection Fail";
 				...
 				if (!AcptNo.substring(4,6).equals("16")){
 					...
 					// 테스트전담여부확인 컬럼
 					strQuery.append(" select count(*) cnt                           \n");
 					strQuery.append("   from cmc0400 a, cmc0420 b,cmr1012 c        \n");           
 					strQuery.append("  where c.cr_acptno=?                         \n");             
 					strQuery.append("    and c.cr_orderid=b.cc_orderid   		   \n");
 					strQuery.append("    and b.cc_reqid = a.cc_reqid               \n");
 					strQuery.append("    and a.cc_srreqid is not null	           \n");
 					...
 					ecamsLogger.error("SRRestApi AcptNo !!! ["+ AcptNo + "] wkFg F 대상아님 [" + wkFg +"] " );
 					...
 					if (!"F".equals(wkFg)){
 						...
 						strQuery.append("  from cmm0015                       \n");		
 						strQuery.append(" where cm_gbncd='SR'                 \n");
 						//strQuery.append(" where cm_gbncd='ST'                 \n");						
 						pstmt = conn.prepareStatement(strQuery.toString());	
 						//pstmt = new LoggableStatement(conn,strQuery.toString());
 						rs = pstmt.executeQuery();
 						if (rs.next()){
 							//https 로 변경될 경우 대비 해야함(cmm0010 table real)
 							srURL = "http://" + rs.getString("cm_svrip") +"/srs/ecams/v1/deployResult";
 						}
 						rs.close();
 						pstmt.close();			
 						...
 						strQuery.setLength(0);
 						strQuery.append("select to_char(a.cr_acptdate,'yyyy-mm-dd hh24:mi:ss') cr_acptdate,  \n");
 						strQuery.append("       to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss') cr_prcdate,        \n");
 						strQuery.append("       a.cr_editor                   \n");
 						strQuery.append("from cmr1000 a                       \n");		
 						strQuery.append("where a.cr_acptno = ?                \n");
 						pstmt = conn.prepareStatement(strQuery.toString());	
						...
						pstmt.close();
 						...
 						ecamsLogger.error("SRRestApi callTSRResult= jsonStr["+ AcptNo +jsonStr.toString());
 						//ecamsLogger.error("SRRestApi callTSRResult= UTF8["+ AcptNo +URLEncoder.encode(jsonStr,"UTF-8") );
 						//ecamsLogger.error("SRRestApi callTSRResult= EUKR["+ AcptNo +URLEncoder.encode(jsonStr,"EUC-KR") );
 						...
 						gitBaseCmd = "curl -v --request POST " + srURL + " --header 'Content-Type: application/json; charset=utf-8' --data-raw " + "'" + jsonStr + "'" ;
 						ecamsLogger.error("SRRestApi callTSRResult= gitBaseCmd  "+ gitBaseCmd);
 						...
 						retMsg = "OK";
 						ecamsLogger.error("SRRestApi callTSRResult= retCd  "+ retMsg);
 					}
 				}
 			}
 			conn.close();
 			...
 			sqlexception.printStackTrace();
 			ecamsLogger.error("## SRRestApi.callTSRResult() SQLException START ##");
 			ecamsLogger.error("## Error DESC : ", sqlexception);	
 			ecamsLogger.error("## SRRestApi.callTSRResult() SQLException END ##");			
 			throw sqlexception;
 		} catch (Exception exception) {
 			exception.printStackTrace();
 			ecamsLogger.error("## SRRestApi.callTSRResult() Exception START ##");				
 			ecamsLogger.error("## Error DESC : ", exception);	
 			ecamsLogger.error("## SRRestApi.callTSRResult() Exception END ##");				
 			throw exception;
 		}finally{
 			if (strQuery != null) 	strQuery = null;
 			if (rs != null)     try{rs.close();}catch (Exception ex){ex.printStackTrace();}
 			...
 			if (conn != null){
 				try{
 					...
 					ecamsLogger.error("## SRRestApi.callTSRResult() connection release exception ##");
 					ex3.printStackTrace();
 				}
 			}
 		}
 		return retMsg;
 	}//end of callSRResult() method statement
    
    // 테스트적용시 SR연계 api호출

    
	public String callSRTestYn(String AcptNo,ArrayList<HashMap<String, String>> srList,Connection conn) throws SQLException, Exception{
	PreparedStatement pstmt       = null;
	ResultSet         rs          = null;
	StringBuffer      strQuery    = new StringBuffer();
	String            retMsg      = "";
	...
	boolean   findSw = false;
	boolean  errSw = false;
	HashMap<String,String> srData = new HashMap<String,String>();
	...
	try {
		...
		if (!findSw) {
			retMsg = "ER:해당 신청번호로 신청기록정보가 없습니다. ["+AcptNo+"]";
			errSw = true;
		}
		...
		if (!errSw){
			...
			rs = pstmt.executeQuery();
			if (rs.next()){
				srURL = "http://" + rs.getString("cm_svrip") +"/srs/ecams/v1/validationResult";
			}
			rs.close();
			pstmt.close();			
			
			if (srURL.length()==0) {
				retMsg = "ER:SR연결정보가 없습니다. [관리자>환경설정>작업서버정보]";
				errSw = true;
			}
		}
		if (!errSw) {	
			findSw = false;
			jsonAry = new JsonArray();
			for (int i=0;srList.size()>i;i++) {
				if (srList.get(i).get("cc_testreq_yn") != null && "Y".equals(srList.get(i).get("cc_testreq_yn"))) {
					jsonObj2 = new JsonObject();
					jsonObj2.addProperty("SrDevReqNo", srList.get(i).get("cc_srreqid"));
					jsonAry.add(jsonObj2);
					findSw = true;
				}
			}
			...
			if (findSw) {
				...
				jsonStr = gson.toJson(jsonObj);
				jsonObj = null;
				HttpCall_New httpcall = new HttpCall_New();
				srData.put("remoteURL", srURL);
				srData.put("reqMethod", "POST");
				srData.put("sendData", jsonStr);

				ecamsLogger.error("SRRestApi callSRTestYn=ACPTNO ["+AcptNo+"] " + srURL + jsonStr);
				
				retMsg = httpcall.httpCall_common(srData);
				ecamsLogger.error("3937 line SRRestApi callSRTestYn=ACPTNO ["+AcptNo+"] jsonObj :" + retMsg);
				if (retMsg.length()>2 && "{".equals(retMsg.substring(0,1))) {
					jsonObj = new JsonObject();							
					jsonObj = (JsonObject) jsonparser.parse(retMsg);
					if (jsonObj.has("ValidateCd")) {
						retMsg = "OK:"+jsonObj.get("ValidateCd").toString().replace("\"", "");;
					} 
				}
			} else {
				retMsg = "NONE";
		...
		ecamsLogger.error("SRRestApi callSRTestYn=ACPTNO ["+AcptNo+"] " + retMsg);
		//conn.close();
		//conn = null;
		pstmt = null;
		rs = null;	
		...
		sqlexception.printStackTrace();
		ecamsLogger.error("## SRRestApi.callSRTestYn() SQLException START ##");
		ecamsLogger.error("## Error DESC : ", sqlexception);	
		ecamsLogger.error("## SRRestApi.callSRTestYn() SQLException END ##");	
		retMsg ="ER:"+sqlexception;
	} catch (Exception exception) {
		exception.printStackTrace();
		ecamsLogger.error("## SRRestApi.callSRTestYn() Exception START ##");				
		ecamsLogger.error("## Error DESC : ", exception);	
		ecamsLogger.error("## SRRestApi.callSRTestYn() Exception END ##");				
		retMsg ="ER:"+exception;
	}finally{
		if (strQuery != null) 	strQuery = null;
		if (rs != null)     try{rs.close();}catch (Exception ex){ex.printStackTrace();}
		...
		if (conn != null){
			try{
				...
				ecamsLogger.error("## SRRestApi.callSRTestYn() connection release exception ##");
				ex3.printStackTrace();
			}
		}
		*/
	...
	return retMsg;
	}//end of callSRTestYn() method statement
    ...
    public String callTSRResult_srreqid(String AcptNo) throws SQLException, Exception{
		//gbnCd  1:테스트적용결과/2:운영적용결과/3:테스트완료여부검증 하고자 함
			...
			if (conn == null) retMsg = "ERDB00:DB Connection Fail";
				...
				if (!AcptNo.substring(4,6).equals("16")){
					...
					if (!"F".equals(wkFg)){
						...
						strQuery.setLength(0);
						// cmr1012 에 cc_srreqid 칼럼추가하면 좋을듯 
						strQuery.append("select distinct cc_srreqid  \n");				
						strQuery.append("  from cmc0400 a,cmc0420 b,cmr1012 c     \n");		
						strQuery.append(" where c.cr_acptno=?                     \n");	
						strQuery.append("   and c.cr_orderid=b.cc_orderid         \n");
						strQuery.append("   and a.cc_reqid = b.cc_reqid           \n");
						strQuery.append("    and a.cc_testreq_yn = 'Y'            \n");
						pstmt = conn.prepareStatement(strQuery.toString());	
    ...
    public String callSRResult_srreqid(String AcptNo) throws SQLException, Exception{
		//gbnCd  1:테스트적용결과/2:운영적용결과/3:테스트완료여부검증 하고자 함
		...
		try {
				...
				if (!AcptNo.substring(4,6).equals("16")){
					...
					if (!"F".equals(wkFg)){
						...
						*/
						...
						// cmr1012 에 cc_srreqid 칼럼추가하면 좋을듯 
						strQuery.append("select distinct (select cc_srreqid from cmc0400 where cc_reqid = b.cc_reqid) cc_srreqid  \n");				
						strQuery.append("  from cmc0420 b,cmr1012 c                   \n");		
						strQuery.append(" where c.cr_acptno=?                         \n");	
						strQuery.append("   and c.cr_orderid=b.cc_orderid             \n");
			...
			if (conn != null){
				try{
					...
					ecamsLogger.error("## SRRestApi.callSRResult() connection release exception ##");
					ex3.printStackTrace();
				}
			}
		}
