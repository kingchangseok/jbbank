...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
			...
			dragEnd: function( events ) {
				...
				if(this.resizing) {
					...
					if(p.forceFit===true){
						...
						// 그리드 마지막컬럼 100% 채우기
						// 그리드 마지막 컬럼 index 구하기
						var lvc = p.colModel.length-1;
						// 마지막 컬럼 기본값 구하기
						var maxLastWidth = parseInt($(this.bDiv).width() * Number(p.colModel[lvc].widthOrg) / 100,10);
						
						// 스크롤 width 구분(IE는 scroll width 포함 으로 0)
						var scrollWidth = 16;
						if(isMSIE) scrollWidth = 0;
						
						// 그리드가 틀보다 작아질경우 (공백이 생길 때)
						if(p.tblwidth < $(this.bDiv).width()-scrollWidth){
							
							// 공백 길이 구하기
							var maxWidth = $(this.bDiv).width() - scrollWidth - p.tblwidth;
							// 마지막 컬럼의 현재 width 가 초기값보다 클경우 ( 사이즈 변동이 일어난 후 )
							if(p.colModel[lvc].width >  maxWidth){
								maxLastWidth = p.colModel[lvc].width;
							}
							
							// 기본값 + 공백 길이
							maxWidth = parseInt(maxLastWidth + maxWidth,10);
							
							// 마지막 컬럼 width 조절
							p.colModel[lvc].width = maxWidth;
							this.headers[lvc].width = maxWidth;
							this.headers[lvc].el.style.width = maxWidth + "px";
							this.cols[lvc].style.width = maxWidth+"px";
							if(this.footers.length>0) {this.footers[lvc].style.width = maxWidth+"px";}
							if(this.hrheaders.length>0) {this.hrheaders[lvc].style.width = maxWidth+"px";}

							$('table:first',this.bDiv).css("width",$(this.bDiv).width()-scrollWidth+"px");
							$('table:first',this.hDiv).css("width",$(this.bDiv).width()-scrollWidth+"px");
							p.tblwidth = $(this.bDiv).width()-scrollWidth;
							$(ts).triggerHandler("jqGridResizeStop", [maxWidth, lvc]);

							// 그리드가 틀보다 커질경우 (스크롤이 생길 때)
						} else {
							
							// 그리드와 틀 사이 값 구하기 ( 스크롤 생기는 크기 )
							var diffWidth = $(this.bDiv).width()-scrollWidth - p.tblwidth;
							
							if(p.colModel[lvc].width >  maxLastWidth) {
								// diffWidth = -값
								// 마지막 컬럼이 스크롤 값보다 큼 ( 스클롤 값보다 작으면(-), 다른 컬럼이 늘어난 것으로 100%를 맞추기 위해 기본사이즈 값으로 들어가야함)
								// 마지막 컬럼 기본값이 스크롤 값보다 큼 ( 스크롤 값보다 작으면, 다른 컬럼이 늘어난 것으로 100%를 맞추기 위해 기본사이즈 값으로 들어가야함)
								if(diffWidth < 0 && p.colModel[lvc].width + diffWidth > 0 && p.colModel[lvc].width + diffWidth > maxLastWidth ){
									// 마지막 컬럼의 사이즈 = 마지막 컬럼의 현재 width - 스크롤 값
									maxLastWidth = p.colModel[lvc].width + diffWidth;

									// 스크롤이 생긴게 마지막 컬럼의 width 때문에 생겼을때
								} else {
									// 현재 width - 기본값 = 스크롤 값
									diffWidth = maxLastWidth - p.colModel[lvc].width;
								}

								p.colModel[lvc].width = maxLastWidth;
								this.headers[lvc].width = maxLastWidth;
								this.headers[lvc].el.style.width = maxLastWidth + "px";
								this.cols[lvc].style.width = maxLastWidth+"px";
								if(this.footers.length>0) {this.footers[lvc].style.width = maxLastWidth+"px";}
								if(this.hrheaders.length>0) {this.hrheaders[lvc].style.width = maxLastWidth+"px";}

								p.tblwidth = p.tblwidth + diffWidth;
								$('table:first',this.bDiv).css("width",p.tblwidth+"px");
								$('table:first',this.hDiv).css("width",p.tblwidth+"px");
								$(ts).triggerHandler("jqGridResizeStop", [maxLastWidth, lvc]);
							}
			...
			selectionPreserver : function(ts) {
				...
				restoreSelection = function() {
					...
					if(p.multiselect && sra && sra.length>0) {
						var selList = sra.join("|");
						selList = "|" + selList + "|";

						var sortIndex = ts.p._sortIndex;
						
						for( i = ts.p.pageStart; i<= ts.p.pageEnd; i++){
							var targetJqId = ts.p.data[ts.p._index[i]].jqId;
							
							if ( sortIndex.length==0 && selList.indexOf("|"+ targetJqId + "|") > -1 ){
								$(ts).jqGrid("setSelection", targetJqId, false, "_sp_");
							} else if ( sortIndex.length> 0 && selList.indexOf("|"+ sortIndex[i-1] + "|") > -1 ) {
								$(ts).jqGrid("setSelection", sortIndex[i-1], false, "_sp_");
							}
						}
		...
		addMulti = function(rowid, pos, irow, checked, uiclass, cur){
			// 그리드 데이터에서 __disable_selection__ 일때 체크박스 disabled 설정
			var disabled = "";
			if(cur.__disable_selection__ != undefined && cur.__disable_selection__ == true){
				disabled = "disabled = true";
			}
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" "+uiclass+" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+ " " + disabled + "/>",
		...
		constructTr = function(id, hide, classes, rd, cur ) {
			...
			return '<tr role="row" id="' + cur.jqId + '" tabindex="' + tabindex + '" class="' + classes + '"' +
		...
		addJSONData = function(data, rcnt, more, adjust) {
			...
			if (!drows) { drows = []; }
			len = drows.length;
			// 그리드 추가 후 스크롤 시 No 추가해서 가져오기 ( 마지막 페이지가 아니라면)
			var emptyRowCnt = ts.p.lastpage != ts.p.page ?ts.p.rowNum - drows.length:0;
//			len = ts.p.rowNum;
			i = emptyRowCnt;
			ir = emptyRowCnt;
			len = len + emptyRowCnt;
			...
			while (i<len) { //len -> data.length
				
				cur = drows[i-emptyRowCnt]; // data
				idr = $.jgrid.getAccessor(cur,idn);
				if(idr === undefined) {
					...
					if(idr === undefined) {
						idr = br+i;
						if(ts.p.scroll && ts.p.page > 1){
							idr = idr + ((ts.p.page-1) * ts.p.rowNum);
						}
						if(f.length===0){
							if(dReader.cell){
...
//				idr  = ts.p.idPrefix + idr;
				...
				if( gi ){
					rowData.push( addMulti(idr, ni, i, selr, mlc, cur) );
				}
				if( si ) {
			...
			// 그리드 row 채우기
			var gridMaxRow = Math.ceil(ts.p.height/21);
			if(ts.p.data.length < gridMaxRow){
				var len = ts.p.data.length;
				for(var i=0; i<gridMaxRow - len; i++){
					
					var lastHeight = Math.round(ts.p.height)%20;
					lastHeight = lastHeight < 0 ? Math.ceil(lastHeight) : Math.floor(lastHeight); 
					var last = i == gridMaxRow - len-1?"height: "+ (lastHeight > 10? lastHeight : 20 - lastHeight) +"px !important;":"";
					rowData.push("<tr class='emptyRow "+classes+"'"+(last != ""?" style='"+last+"'":"")+"  id='"+(i+len+1)+"' >");

					if( ni ) {
						rowData.push( '<td role="gridcell"'+(last != ""?" style='"+last+"'":"")+'></td>' );
					}
					if( gi ){
						rowData.push( '<td role="gridcell"'+(last != ""?" style='"+last+"'":"")+'></td>' );
					}
					...
					for (j=0;j<rowReader.length;j++) {
						var hiddenCk = ts.p.colModel[j].hidden == true?" style='display:none; "+last+"'":last != ""?" style='"+last+"'":"";
						rowData.push( '<td role="gridcell"'+hiddenCk+' ></td>' );
					}
					rowData.push( "</tr>" );
				}
			}
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					tablebody.append(rowData.join('')); // 데이터 삽입
		...
		addLocalData = function( retAll ) {
			...
			// sort 상태 일때 sortIndex 따로 저장
			if(st && ts.p.sortorder && fndsort && queryResults.length > 0){
				var sortIndex = [];
				$(queryResults).each(function(){
					sortIndex.push(this.jqId);
				});
				
				ts.p._sortIndex = sortIndex;
			}
			...
			if(ts.p.treeGrid && ts.p.search) {
				...
				if( ts.p.pageEnd % recordsperpage > 0 && ts.p.pageEnd > recordsperpage && ts.p.pageEnd != ts.p.data.length ){
					queryResults = queryResults.slice( ((page-1)*recordsperpage) + (ts.p.pageEnd % recordsperpage) , page*recordsperpage );
				} else {
		...
		updatepager = function(rn, dnd, addRowData) {
			...
			// 이전 페이지 * 한페이지 row개수 = 현재 그려진값의 처음 값
			base = base*parseInt(ts.p.rowNum,10);
			
			// 마지막 페이지에 그려질 값 ts.p.reccount = 페이지에 그려진값
			to = base + ts.p.reccount;
			if (ts.p.scroll) {
				var rows = $("tbody:first > tr:gt(0):not(.emptyRow)", ts.grid.bDiv);
				if(to > ts.p.records ) {
					to = ts.p.records;
				}
				base = to - rows.length;
				...
				ts.p.pageEnd = 0;
				if(rows.length > 0){
					ts.p.pageStart = $.inArray(Number(rows[0].id - 1 ), ts.p._index);
					ts.p.pageStart = ts.p.pageStart < 1 ? 1: ts.p.pageStart;
					ts.p.pageEnd = ts.p.pageStart + ts.p.reccount -1;
				}
				
				if(ts.p.pageEnd > ts.p.records || (ts.p.lastpage == ts.p.page && ts.p.pageEnd < ts.p.records)) {
					ts.p.pageEnd = ts.p.records;
					to = ts.p.records;
				}
				...
				if (rh) {
					...
					} else if (ts.p.page == ts.p.lastpage){
						// 마지막 페이지 일때 스크롤 맨밑 고정
						$(">div:first",ts.grid.bDiv).children("div:first").css({height: height});
						ts.grid.bDiv.scrollTop = height+ts.offsetHeight;
					} else if (addRowData && rows[0].id != ts.p._index[ts.p.pageStart]){
						base = ts.p.pageStart-1;
						to = ts.p.pageEnd;
						top = ts.p.pageStart == 1?0:ts.p.pageStart * rh;
						$(">div:first",ts.grid.bDiv).children("div:first").css({height: top, display: top?"":"none"});
					}
				}
			...
			// 스크롤 해서 하단에 데이터 추가 될 때  restoreSelection 호출
			if(ts.p.pageEnd - ts.p.pageStart >  ts.p.rowNum && ts.p.multiselect && ts.p.selarrrow.length > 0){
		...
		setColWidth = function () {
			...
			if( vc > 0) {
				...
				$.each(ts.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						if(ts.p.shrinkToFit===true ){
							...
							cw = Math.round(aw*this.width*0.01);
						}
						this.width =cw;
						initwidth += cw;
						lvc = i;
				...
				if (hs) {
					if(grid.width-gw-(initwidth+brd*vc) !== scw && ts.p.shrinkToFit === true){
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			if(tmpcm.name === "rn")ts.p.colNames[i] = "NO";
		...
		if(this.p.multiselect) {
			...
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).on('click',function(){
				...
				if(ts.p._index.length == undefined || ts.p._index.length == 0){
					return;
				}
				
				var from = ts.p.pageStart,
				to = ts.p.pageEnd,
				sortPageId = "";
				
				if(ts.p.sortname != undefined && ts.p.sortname != "" ){
					var sortIndex = ts.p._sortIndex;
					for(var i=from-1; i < to; i++){
						sortPageId += "|"+sortIndex[i];
					}
					if(sortPageId.length > 0){
						sortPageId += "|";
					}
				}
...
//					$(ts.rows).each(function(i) {
					$(ts.p._index).each(function(i) {
						if(this == undefined || this == null){
							return;
						}
						var targetJqId = ts.p.data[this].jqId;
						
						if( (i >= from && i <= to && sortPageId.length == 0) || (sortPageId.length > 0 && sortPageId.indexOf("|"+targetJqId+"|") > -1) ){
							// disabled 상태일때 체크 안되도록 설정
							var jqId = $("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(targetJqId));
							
							// 그려져 있는 row
							if(jqId.length > 0){
								if( jqId.prop("disabled") != false && jqId.prop("disabled") != "" && jqId.prop("disabled") != undefined ){
									return true;
								}
								if (i>=0) {
									var target = jqId.closest("tr");
									if(!target.hasClass("ui-subgrid") && !target.hasClass("jqgroup") && !target.hasClass(disabled) && !target.hasClass("jqfoot")){
										$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(targetJqId) )[ts.p.useProp ? 'prop': 'attr']("checked",true);
										target.addClass(highlight).attr("aria-selected","true");
										if(ts.p.preserveSelection) {
											if(ts.p.selarrrow.indexOf(targetJqId) === -1) {
												ts.p.selarrrow.push(targetJqId);
											}
										} else {
											ts.p.selarrrow.push(targetJqId);
										}
										ts.p.selrow = targetJqId;
										if(froz) {
											$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(targetJqId), ts.grid.fbDiv )[ts.p.useProp ? 'prop': 'attr']("checked",true);
											$("#"+$.jgrid.jqID(targetJqId), ts.grid.fbDiv).addClass(highlight);
										}
									}
								}
							}
...
//					$(ts.rows).each(function(i) {
						...
						if( (i >= from && i <= to && sortPageId.length == 0) || (sortPageId.length > 0 && sortPageId.indexOf("|"+targetJqId+"|") > -1) ){
							...
							if(jqId.length > 0){
								...
								if(i>=0) {
									var target = jqId.closest("tr");
									if(!target.hasClass("ui-subgrid") && !target.hasClass("jqgroup") && !target.hasClass(disabled) && !target.hasClass("jqfoot")){
										$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(targetJqId) )[ts.p.useProp ? 'prop': 'attr']("checked", false);
										target.removeClass(highlight).attr("aria-selected","false");
										emp.push(targetJqId);
										if(ts.p.preserveSelection) {
											var curind = ts.p.selarrrow.indexOf(targetJqId);
											if(curind > -1) {
												ts.p.selarrrow.splice(curind, 1);
											}
										}
										if(froz) {
											$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(targetJqId), ts.grid.fbDiv )[ts.p.useProp ? 'prop': 'attr']("checked",false);
											$("#"+$.jgrid.jqID(targetJqId), ts.grid.fbDiv).removeClass(highlight);
										}
									}
								}
							}
		...
		function selectMultiRow(ri, scb, e, selection) {
			if((ts.p.multiselect && ts.p.multiboxonly) || ts.p.multimail ) {
				...
				// disabled 상태일때 체크 안되도록 설정
				var jqId = $("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+ri); 
				if( jqId.length > 0 && jqId.prop("disabled") != false && jqId.prop("disabled") != "" && jqId.prop("disabled") != undefined ){
					return;
				} else {
					$(ts).jqGrid("setSelection", ri, selection, e);
				}
		...
		$(ts).before(grid.hDiv).on({
			...
			'reloadGrid': function(e,opts) {
				...
				// 그리드 reload 할 때(data를 넣을 때) NO width 조정 
				//ts.p.rownumWidth
				if(ts.p.data.length >= 1000){
					var rowNoWidth = 32+(7 * ((ts.p.data.length+"").length - 3));
					if( ts.p.rownumWidth != rowNoWidth){
						if(ts.p.colModel[0].name == "rn"){
							ts.p.rownumWidth = rowNoWidth;
							// resizeable = true 를 넣어 rn 컬럼(NO) 만 width 변경
							ts.p.colModel[0].resizable = true;
							$(ts).jqGrid('resizeColumn', 'rn', rowNoWidth);
							
							// 그리드 전체 컬럼 width 변경
							ts.p.colModel[0].widthOrg = rowNoWidth;
...
$.jgrid.extend({
		...
		return this.each(function(){
			...
			if (this.grid && typeof newParams === 'object') {
				if(overwrite === true) {
					...
					for(var key in Object.keys(newParams)){
						if(Object.keys(newParams)[key] == "data"){
							$(newParams.data).each(function(i){
								this.jqId = i+1;
							});
						}
					}
	...
	getGridRowById : function ( rowid ) {
		...
		/*
		//ID 가 없으면 강제 스크롤 후 다시 찾음
		if(rowid != null && rowid != undefined && this != undefined && this != null && this.getGridParam("data").length >= Number(rowid) && (row == undefined || row == null)){
			this.each( function(){
				
					var reScrollTop = this.grid.bDiv.scrollTop;
					$(this.grid.bDiv).scrollTop(( (rowid-1) * 20));
					$(this.grid.bDiv).off("scroll");
					this.grid.populateVisible();
//					$(this.grid.bDiv).scrollTop(reScrollTop);
					$(this.grid.bDiv).on("scroll",this.grid.scrollGrid);
	...
	setSelection : function(selection,onsr, e, isHight) {
		return this.each(function(){
			...
			if($t.p.data.length < Number(selection)) return;
...
//				$t.setHeadCheckBox( false );
				$t.p.selrow = pt.id;
				ia = $.inArray(Number($t.p.selrow),$t.p.selarrrow);
				if (  ia === -1 ){
					if(pt.className !== "ui-subgrid") { $(pt).addClass(highlight).attr("aria-selected","true");}
					stat = true;
					$t.p.selarrrow.push(Number($t.p.selrow));
				} else if( ia !== -1 && e === "_sp_") { 
	...
	addRowData : function(rowid,rdata,pos,src) {
		...
		var success = false, nm, row, rnc="", msc="", gi, si, ni,sind, i, v, prp="", aradd, cnm, data, cm, id, jqId;
		if(rdata) {
			if($.isArray(rdata)) {
				aradd=true;
				//pos = "last";
...
//					id = rowid;
					...
					if(t.rows.length === 0){
						...
						switch (pos) {
							case 'last':
								id = t.p.data.length+1;
								if(t.p.data.length == t.p.pageEnd){
									addRowCk = true;
									if(t.p.data.length+1 < t.rows.length){
										$(t.rows[t.p.data.length]).after(row.join(''));
										$(t.rows[t.p.data.length+2]).remove();
									} else {
							...
							case 'first':
								id = 1;
								if(t.p.data.length == 0){
									addRowCk = true;
									$(t.rows[0]).after(row.join(''));
									$(t.rows[2]).remove();
								} else if (t.p.data[Number(t.p._index[1])].jqId == t.rows[1].id) {
									addRowCk = true;
									$(t.rows[0]).after(row.join(''));
									if(t.p.data.length+1 < t.rows.length){
										var delRow = t.rows.length-2;
										if(t.p.data.length+3 == t.rows.length) {
											delRow = t.rows.length-1;
										}
										if(t.rows[delRow].className.indexOf("emptyRow") > -1){
											$(t.rows[delRow]).remove();
										}
									}
								}
								sind = 1;
								break;
							case 'after':
								sind = $(t).jqGrid('getGridRowById', t.p._index[src]+1);
								if (sind) {
									addRowCk = true;
					...
					if(t.p.datatype === 'local') {
						...
						if(t.p._index.length > -1){
							t.p._index.splice(id,0,data.jqId-1);
						} else {
							t.p._index = [];
							t.p._index.push({});
							t.p._index.push(data.jqId-1);
						}
...
//						t.p.data.push(lcdata);
				...
				t.updatepager(true,true,pos != "last"?true:false);
				success = true;
			});
		}
		return success;
		...
		return this.each(function(){
			...
			setTimeout(function(){
				try {
					...
					$("#"+$.jgrid.jqID($t.p.id)).jqGrid('setGridWidth', parentwidth-5);
				} catch(e){}
			}, timeout);
		});
	},
					...
					switch (cm.edittype) {
						...
						case 'select':
							if(!cm.editoptions.multiple) {
								...
								// inline eidt 할 때 text 를 저장해서 변경
...
$.jgrid.extend({
	...
	exportToExcel : function ( o ) {
		...
		excelExportJqGrid(this,o.fileName);
		/*
		this.each(function() {
			var $t = this,
...
//=========================================================================
			function groupToExcel ( grdata ) {
				...
				$.each(grp.groups,function(i,n){
					...
					var arr;
					if(grp.groupSummaryPos[n.idx] === 'header')  {
						arr = buildSummaryTd(i, 0, grp.groups, 0 );
					} else {
