...
function gridEdit() {
	...
	var lastIndex = grdLst.columns.length -1
	grdLst.removeColumn(lastIndex);
	...
	var lastIndex = grdChk.columns.length -1
	grdChk.removeColumn(lastIndex);
	...
	var lastIndex = grdCond.columns.length -1
	grdCond.removeColumn(lastIndex);
}
...
function grdLst_click() {
	...
	if (grdLst_dp.length > 0){
		...
		$('#txtCase').attr('readonly', true);
		if(grdChk.columns.length > 1 && window.parent.strReqCd != "44") {
			grdChk.removeColumn();
		}
		$('#chkOpen').prop('checked', false);
