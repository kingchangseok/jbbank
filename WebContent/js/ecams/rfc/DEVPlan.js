...
function cboSet() {
	...
	tmpObj = {};
	tmpObj.cm_micode = "01";
	tmpObj.cm_codename = "개발계획등록대상";
	cboQryGbn_dp.push(tmpObj);

	tmpObj = {};
	tmpObj.cm_micode = "02";
	tmpObj.cm_codename = "개발실적등록대상";
	cboQryGbn_dp.push(tmpObj);
...
function successGetSubTab(data) {
	...
	if(tmpSubTab.indexOf("41") >= 0) {
		$("#srTab").fadeTo(0, 1);
		$('#srTab').css('pointer-events', 'visible');	
	} else if(tmpSubTab.indexOf("42") >= 0) {
		$("#srTab").fadeTo(0, 1);
		$('#srTab').css('pointer-events', 'visible');	
	}
	changeTabMenu();
