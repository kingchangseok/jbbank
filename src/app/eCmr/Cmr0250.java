		...
		try {
			...
			if (rs.next()){
				...
				if (rs.getString("cr_status").equals("9")) { //|| rs.getString("cr_status").equals("8")) {
					...
					rsconf = cmr0150.confL
					ocat_conn(AcptNo,strPrcSw,rs.getString("cr_cnclstep"),rs.getString("cncldat"),rs.getString("cr_retryyn"),conn);
					cmr0150 = null;
...
//			strQuery.append("		(select cm_codename from cmm0020 where cm_macode = 'REQPASS' and cm_micode = decode(a.cr_aplydate,null,'0','4') and a.cr_qrycd = '04') aply,	\n"); // 20231130 일반으로만 보임
	...
	public Object[] getQACheck(String Acptno) throws SQLException, Exception {
		...
		try {
        	...
        	if (rsval.size()==1) {
				strQuery.setLength(0);
	        	strQuery.append("select b.cr_qacheck,b.cr_totalcheck,cr_totalcheck, \n");
	        	strQuery.append("       b.cr_chgcode,b.cr_detailcode,  \n");
	        	strQuery.append("       nvl(b.cr_devptime,a.cr_devptime) cr_devptime  \n");	        	
	        	strQuery.append("  from cmr0900 b, cmr1000 a          \n");
	...
	public Object[] getPoc(String AcptNo, String UserId) throws SQLException, Exception {
		...
		try {
			...
			/* 신청한 부서와 동일한 부서로 로그인한 사람인지 확인 추가 */
			strQuery.setLength(0);
			...
			strQuery.append("         from cmm0040 b                    \n");
			strQuery.append("        where b.cm_userid = ?              \n");
			strQuery.append("          and (b.cm_project  = a.cr_teamcd \n");
			strQuery.append("           or b.cm_project2 = a.cr_teamcd) \n");
			strQuery.append("   )                                       \n");
			pstmt = conn.prepareStatement(strQuery.toString());
			pstmt.setString(1, AcptNo);
	       ...
	       // //ecamsLogger.error(((LoggableStatement)pstmt).getQueryString());
			...
			while (rs.next()){
				...
				if(isSameTeamUser > 0){
					rst.put("isSameTeamUser", "true");
				}else{
					rst.put("isSameTeamUser", "false");
				}
...
//			chkAry[1] = "procck2";
	...
	public String progCncl_sel(String AcptNo,ArrayList<HashMap<String,String>> fileList,String PrcSys) throws SQLException, Exception {
	    ...
	    try {
	        ...
	        // 로그 1: 반송 처리 시작
	        ecamsLogger.error("========== [REFUND_START] AcptNo:" + AcptNo 
	                        + ", FileCount:" + fileList.size() + " ==========");
	        
	        for (int i=0;fileList.size()>i;i++) {
	            ////////////////자바에딸린 클래스 찾아서 반려
	            itemId   = fileList.get(i).get("cr_itemid");
	            rsrcName = fileList.get(i).get("cr_rsrcname");
	            
	            // 로그 2: 처리 대상 파일 정보
	            ecamsLogger.error("[REFUND_FILE_" + i + "] ItemId:" + itemId 
	                            + ", FileName:" + rsrcName 
	        ...
	        // 로그 9: 전체 처리 완료
	        ecamsLogger.error("========== [REFUND_COMPLETE] AcptNo:" + AcptNo 
	        ...
	        sqlexception.printStackTrace();
	        // 로그 10: 롤백 발생
	        ecamsLogger.error("[ROLLBACK_SQL] AcptNo:" + AcptNo + ", Error:" + sqlexception.getMessage());
	        ...
	        exception.printStackTrace();
	        // 로그 11: 일반 예외 발생
	        ecamsLogger.error("[ROLLBACK_EXCEPTION] AcptNo:" + AcptNo + ", Error:" + exception.getMessage());
...
//				////////////////자바에딸린 클래스 찾아서 반려
	...
	public Object[] getReqs(String OrderId) throws SQLException, Exception {
		...
		try {
			...
			strQuery.setLength(0);
			strQuery.append("select a.cc_orderid,b.cc_docnum,nvl(b.cc_srreqid,'') cc_srreqid,b.cc_reqid,b.cc_docsubj,b.cc_doctype from cmc0420 a, cmc0400 b 					\n"); 
	...
	public Object[] getSRReqList(HashMap<String,String> etcData) throws SQLException, Exception {
		...
		try {
			...
			/* ---------------------------------------------------------
			* 3. QA 결재 완료 여부 확인
			*    cmr9900 중 cr_team='Q1' AND cr_status='9'
			* --------------------------------------------------------- */
			boolean qaDone = false;
			strQuery.setLength(0);
			strQuery.append("SELECT cr_team, cr_status ")
					.append("  FROM cmr9900            ")
					.append(" WHERE cr_acptno = ?      ")
					.append("   AND cr_team = 'Q1'     ")
					.append("   AND cr_status = '9'    ")
					.append("   AND cr_locat <> '00'   ");					
			pstmt = conn.prepareStatement(strQuery.toString());
			pstmt.setString(1, etcData.get("acptno"));

			rs = pstmt.executeQuery();
			if (rs.next()) {
				qaDone = true;  // Q1 결재 완료
			}
			rs.close();
			pstmt.close();

			String srCallYn = etcData.get("srCallYn"); // 'Y' or 'N'
			...
			//1) 테스트 대상 + QA 결재 완료 → API 호출 X, 무조건 Y

			ecamsLogger.error("Cmr0250.getSRReqList() srrestapi testSw: " + testSw + " qaDone:" + qaDone + " testSw:" + testSw + " srCallYn:" + srCallYn);
			...
			// 2) 테스트 대상이고, 지금 단계가 SYSTS/9 ~ Q1/0 구간(canCallSR=Y) → API 호출
			} else if (testSw && "Y".equals(srCallYn)) {
				rst.put("checkyn", "Y");
				SRRestApi  srrestapi = new SRRestApi();
				String retMsg = srrestapi.callSRTestYn(etcData.get("acptno"),rsval,conn);
				ecamsLogger.error("Cmr0250.getSRReqList() srrestapi.callSRTestYn Call >  " + "retMsg: " +retMsg);
	    	...
	    	try {
	            ...
	            while(rs.next()) {
	    	  		...
	    	  		if (rs.getString("pro_itemid") != null){
	    	  			strQuery.setLength(0);
	    				strQuery.append("select decode(a.cr_qrycd,'03','신규','04','수정','') checkin, \n");
	    				strQuery.append("a.cr_editcon cr_editcon, \n");
						strQuery.append("b.cm_dirpath cm_dirpath \n");
	    				strQuery.append(" from cmr1010	a, cmm0070 b \n");				    		
						strQuery.append("  where a.cr_syscd = b.cm_syscd and a.cr_dsncd = b.cm_dsncd and  a.cr_acptno = ? and a.cr_itemid = ? \n");		
	    				pstmt2 = conn.prepareStatement(strQuery.toString());
	    	    		pstmt2 = new LoggableStatement(conn,strQuery.toString());
	    	    		pstmt2.setString(1, AcptNo);
