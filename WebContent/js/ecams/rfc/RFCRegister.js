...
$(document).ready(function() {
	...
	$('#tabSRAccept').width($('#tabSRAccept').width()+10);
	$("#tabRFCRegister").show();	
	...
	document.getElementById('frmRFCRegister').onload = function() {
		getTeamInfoGrid2();
	}
});
...
function cboSet() {
	...
	tmpObj = {};
	tmpObj.cm_micode = "01";
	tmpObj.cm_codename = "RFC발행대상";
	cboQryGbn_dp.push(tmpObj);

	tmpObj = {};
	tmpObj.cm_micode = "02";
	tmpObj.cm_codename = "RFC발행중";
	cboQryGbn_dp.push(tmpObj);
...
function iSRID_Click(data) {
	...
	tab1 = $('#frmSRRegister').get(0).contentWindow;
	tab2 = $('#frmSRAccept').get(0).contentWindow;
	tab2 = $('#frmRFCRegister').get(0).contentWindow;
	tabBase = $('#frmBaseISRInfo').get(0).contentWindow;
	...
	//그리드 row 선택시 RFC발행 탭으로 변경
	if(!$("ul.tabs li#tab3").hasClass("on")) {
		$(".tab_content").hide();
		var activeTab = $("ul.tabs li#tab3").attr("rel");
		
		//tab메뉴 클릭에 따라 색상 변경
		$("ul.tabs li").removeClass('on');
		$("ul.tabs li#tab3").addClass("on");
...
function confCall(GbnCd) {
	...
	confirmInfoData = {		
		UserId : tab3.strUserId,
		ReqCd  : tab3.strReqCd,
		SysCd  : "99999",
		RsrcCd : [],
		EmgSw : "N",//긴급여부
		PrjNo : tab3.strIsrId+tab3.strIsrSub,
		QryCd : [],
		PgmType : "",
		deployCd : ""
	}
	...
	if(GbnCd == "Y") {
	        ...
	        onStateChanged: function () {
	            if (this.state === "open") {
	            	...
	            	if(confirmData.length > 0){
	            		tab3.reqQuest(confirmData, this.state);
	            	}
	                mask.close();
					tab3.ingSw = false;
	            }
		...
		ajaxAsync('/webPage/request/ConfirmServlet', tmpData, 'json', tab3.successConfirm_Info);
	}
}