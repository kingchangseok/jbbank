...
public class eCAMSBaseServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			case "LOG_OUT":
				...
				break;
			case "CHECK_SECU":/* 취약점수정 (2025) */
				//response.getWriter().write( chkMenuSecu(jsonElement) );
				response.getWriter().write( chkMenuSecu(jsonElement, request) );
				break;
			default:
				throw new Exception("Servlet Function 誘� 議댁옱");
			}
	...
	private String getSessionUserData(JsonElement jsonElement, HttpServletRequest request) throws SQLException, Exception {
		...
		if (null == jwtStr || "".equals(jwtStr)) {
			...
			if (userId == null || "".equals(userId) ) {
		    	...
		    	String token = Jwts.builder().setClaims(claims2).setIssuedAt(now).setExpiration(validity)
						.signWith(SignatureAlgorithm.HS256, TokenSecretKey.getSecretKey()).compact(); //占쎈쐻占쎈짗占쎌굲占쎈쐻占쎈짗占쎌굲占쎈쐻�뜝占� 占쎈쐻占쎈짗占쎌굲占쎈쐻占쎈짗占쎌굲
		...
		if (null != jwtStr && !"".equals(jwtStr) && null != UserID && !"".equals(UserID)) {
		    ...
		    if (userData != null) {
		    	userInfoMap.put("position", userData.get("cm_position"));//筌욊낯�맄占쎌젟癰귨옙
	...
	/* 취약점수정 (2025) start*/
	//private String chkMenuSecu(JsonElement jsonElement) throws SQLException, Exception {

	private String chkMenuSecu(JsonElement jsonElement, HttpServletRequest request) throws SQLException, Exception {
		MenuList menulist = new MenuList();
		try {
			String userId = ParsingCommon.jsonStrToStr( ParsingCommon.jsonEtoStr(jsonElement, "userId") );
			String url = ParsingCommon.jsonStrToStr( ParsingCommon.jsonEtoStr(jsonElement, "pathname") );
			//return ParsingCommon.toJson( menulist.chkMenuSecu(userId,url) );
			return ParsingCommon.toJson( menulist.chkMenuSecu(request,url) );
//			ecamsLogger.error("eCAMSBaseServlet.chkMenuSecu()="+userId + url);
